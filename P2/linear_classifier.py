...
def svm_loss_vectorized(
    ...
    # margins = torch.max(torch.zeros_like(scores), scores - scores[range(num_train),y].reshape(-1,1) + 1)
    # margins[range(num_train),y] = 0
    # # print(W*W)
    # loss = torch.sum(margins) / num_train + 0.5 * reg * torch.sum(W * W)
    ...
    # row_sum = torch.sum(margins, axis=1)                  
    # margins[torch.arange(num_train), y] = -row_sum    
    # dW += torch.mm(X.T, margins)/num_train + reg * W
